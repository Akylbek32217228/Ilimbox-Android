...
class CourseModuleDelegate(
    ...
    private fun bindClickListeners(vh: CourseModuleViewHolder, module: Module, position: Int) {
        vh.clickWrapper.setOnClickListener {
            Log.d("ololo", "clickwrapper listener")
            clickWrapperClickListener.onClick(module, position) }
        vh.more.setOnClickListener {
            Log.d("ololo", "more listener")
            moreOptionsClickListener.onClick(module, position)
        }
    }
    ...
    internal class CourseModuleViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val description: CollapsibleTextView = itemView.findViewById(R.id.web_view_section)
